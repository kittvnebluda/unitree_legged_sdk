...
typedef struct {
    ...
    int8_t temperature;
} IMU; // when under accelerated motion, the attitude of the robot calculated by
       // IMU will drift.
...
typedef struct {
    ...
    float ddq_raw;
    int8_t temperature; // current temperature (temperature conduction is slow
                        // that leads to lag)
    // uint32_t reserve[2];
...
typedef struct {
    ...
    uint8_t mode;
    IMU imu;
    // float position[3];                 // (unit: m), from robot own odometry in
    // inertial frame, usually drift
    std::array<float, 3> position;
    // float velocity[3];                 // (unit: m/s), forwardSpeed, sideSpeed,
    // updownSpeed in body frame
...
typedef struct {
    ...
    uint32_t SN;
    uint8_t bandWidth;
    uint8_t mode;           // 0.idle, default stand | 1.force stand (controlled by
                            // dBodyHeight + rpy) 2.target velocity walking (controlled by
                            // velocity + yawSpeed) 3.target position walking (controlled by
                            // position + rpy[2])
                            // 4. path mode walking (reserve for future release)
                            // 5. position stand down. |6. position stand up |7. damping
                            // mode | 8. recovery mode
    uint8_t gaitType;       // 0.trot | 1. trot running  | 2.climb stair
    uint8_t speedLevel;     // 0. default low speed. 1. medium speed 2. high speed.
                            // during walking
    float dFootRaiseHeight; // (unit: m), swing foot height adjustment from
                            // default swing height.
    float dBodyHeight;      // (unit: m), body height adjustment from default body
                            // height.
    // float position[2];                 // (unit: m), desired x and y position
    // in inertial frame.
    std::array<float, 2> position;
    // float rpy[3];                      // (unit: rad), desired yaw-pitch-roll
    // euler angle, expressed in roll(rpy[0]) pitch(rpy[1]) yaw(rpy[2])
    std::array<float, 3> rpy;
    // float velocity[2];                 // (unit: m/s), forwardSpeed, sideSpeed
    // in body frame.
