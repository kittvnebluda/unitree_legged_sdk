...
class Custom {
  public:
    Custom(uint8_t level)
        : safe(LeggedType::Aliengo), udp(LOCAL_PORT, TARGET_IP, TARGET_PORT, sizeof(HighCmd), sizeof(HighState)) {
        udp.InitCmdData(cmd);
    }
    void UDPRecv();
    void UDPSend();
...
void Custom::RobotControl() {
    ...
    std::cout << " A " << ((int)_keyData.btn.components.A == 1) << " B " << ((int)_keyData.btn.components.B == 1)
              << " X " << ((int)_keyData.btn.components.X == 1) << " Y " << ((int)_keyData.btn.components.Y == 1)
              << " lx " << _keyData.lx << " ly " << _keyData.ly << " rx " << _keyData.rx << " ry " << _keyData.ry
              << std::endl;
}
